...
const CommitteeSection = ({ title, members }) => (
  <div className="mb-8">
    <h3 className="text-xl sm:text-2xl font-semibold text-gray-800 mt-4">{title}:</h3>
...
export default function Conference() {
  ...
  return (
    <>
      <Navbar />
      <div className="bg-gray-400 min-h-screen py-4 sm:py-8 px-4">
        <div className="max-w-7xl mx-auto">
            ...
            {hotels.map((hotel, index) => (
              ...
              >
                <div className="mb-2 sm:mb-4">
                  <h2 className="text-lg sm:text-2xl font-bold text-gray-900 tracking-wide hover:text-blue-600">
                    {hotel.name}
                  </h2>
                </div>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md flex-1">
              ...
              {committee && committee["Organizing Committee"] && (
                <div>
                  ...
                  {["Organizing Chairman", "Organizing Co-Chairman"].map((role, index) => (
                    <CommitteeSection
                      key={index}
                      title={role}
                  ...
                  {Object.keys(committee["Organizing Committee"])
                    .filter((role) => Array.isArray(committee["Organizing Committee"][role]))
                    .map((role) => (
                      <CommitteeSection
                        key={role}
            ...
            <div className="bg-white p-6 rounded-lg shadow-md flex-1">
              ...
              {committee && committee["Conference Committee Chairmans & Co-Chairmans"] && (
                <div>
                  {Object.keys(committee["Conference Committee Chairmans & Co-Chairmans"]).map(
                    (role, index) => (
                      <CommitteeSection
                        key={index}
                        title={role.replace(/([A-Z])/g, " $1").trim()}
                        members={committee["Conference Committee Chairmans & Co-Chairmans"][role]}
                      />
                    )
                  )}
                </div>
